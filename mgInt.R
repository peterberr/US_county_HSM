...
ins_div<-c(1.25,1,1.25,1.25,0.65,0.75,0.5,0.75,0.75)
...
# Arch 24, from archs 45 
mgi_all24<-data.frame(Foundation="Crawlspace",Type="SF",Stories="One",Garage="Yes",Size="Large",Div=frame_weights$Div)
mgi_all24[,paste("mi_", mi_aa_1$Material, sep="")]<-as.matrix(frame_weights[,c("Wood","Mason")])%*%t(cbind(mi_aa_45$mi,mi_aa_38$mi))
mgi_all24$mi_Insulation<-ins_div*mgi_all24$mi_Insulation # adjust insulation values based on local climates, by division
mgi_all24$mi_Tot<-rowSums(mgi_all24[,paste("mi_", mi_aa_2$Material, sep="")])
mgi_all24[,paste("gi20_", gi20_aa_1$Material, sep="")]<-as.matrix(frame_weights[,c("Wood","Mason")])%*%t(cbind(gi20_aa_45$gi20,gi20_aa_38$gi20))
mgi_all24$gi20_Insulation<-ins_div*mgi_all24$gi20_Insulation # adjust insulation values based on local climates, by division
mgi_all24$gi20_Tot<-rowSums(mgi_all24[,paste("gi20_", gi20_aa_2$Material, sep="")])
mgi_all24[,paste("gi60_", gi60_aa_1$Material, sep="")]<-as.matrix(frame_weights[,c("Wood","Mason")])%*%t(cbind(gi60_aa_45$gi60,gi60_aa_38$gi60))
mgi_all24$gi60_Insulation<-ins_div*mgi_all24$gi60_Insulation # adjust insulation values based on local climates, by division
mgi_all24$gi60_Tot<-rowSums(mgi_all24[,paste("gi60_", gi60_aa_2$Material, sep="")])
mgi_all24$arch<-24
